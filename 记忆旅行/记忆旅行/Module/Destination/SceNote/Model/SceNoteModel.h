//
//  SceNoteModel.h
//  记忆旅行
//
//  Created by DreamItPossible on 2017/2/19.
//  Copyright © 2017年 Jiang. All rights reserved.
//

#import "BaseModel.h"

@interface SceNoteModel : BaseModel

@property (nonatomic,copy)   NSString *modifyDate;
@property (nonatomic,copy)   NSString *kilometreSum;
@property (nonatomic,copy)   NSString *isshow;
@property (nonatomic,copy)   NSString *userID;
@property (nonatomic,copy)   NSString *lon;
@property (nonatomic,copy)   NSString *Sex;
@property (nonatomic,copy)   NSString *score;
@property (nonatomic,copy)   NSString *YSofficial;
@property (nonatomic,copy)   NSString *traceName;
@property (nonatomic,copy)   NSString *reviews;
@property (nonatomic,copy)   NSString *fictitiousReviews;
@property (nonatomic,copy)   NSString *type;
@property (nonatomic,copy)   NSString *fictitiousScore;
@property (nonatomic,copy)   NSString *frameState;
@property (nonatomic,copy)   NSString *ID;
@property (nonatomic,copy)   NSString *traceTime;
@property (nonatomic,copy)   NSString *Nickname;
@property (nonatomic,copy)   NSString *picShortPath;
@property (nonatomic,copy)   NSString *kidneyname;
@property (nonatomic,copy)   NSString *commens;
@property (nonatomic,copy)   NSString *vote;
@property (nonatomic,copy)   NSString *lat;
@property (nonatomic,copy)   NSString *Photo;
@property (nonatomic,copy)   NSString *Age;
@property (nonatomic,copy)   NSString *TouristID;
@property (nonatomic,copy)   NSString *traceID;
@property (nonatomic,copy)   NSString *Telephone;
@property (nonatomic,copy)   NSString *creatDate;

@end
