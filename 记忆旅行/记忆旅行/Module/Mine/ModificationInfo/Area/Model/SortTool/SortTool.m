...
+ (nullable NSArray *)sortObjcs:(nonnull NSArray <NSObject *>*)sources byKey:(nonnull NSString *)key withSortType:(SortResultType)sortType {
    ...
    if (sortType == SortResultTypeDoubleValues) {
        NSMutableArray *array = [NSMutableArray arrayWithCapacity:0];
        //遍历已排序的字母数组
        for (int i = 0;i < lqq_resultArray.count;i++) {
            NSString *key = [lqq_resultArray objectAtIndex:i];
            NSMutableDictionary *dic = [[NSMutableDictionary alloc]init];
            [dic setObject:key forKey:SortToolKey];
            [dic setObject:[arr objectAtIndex:i] forKey:SortToolValueKey];
            [array addObject:dic];
        }
        return array;
    } else {
...
+ (NSArray *)SortDataByFirstLetterWithArray:(NSArray <NSString *>*)sourceArray {
